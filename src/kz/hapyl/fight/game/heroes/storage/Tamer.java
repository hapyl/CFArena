...
import kz.hapyl.fight.game.heroes.storage.extra.TamerPack;
import kz.hapyl.fight.game.heroes.storage.extra.TamerPacks;
...
public class Tamer extends Hero implements Listener {
	...
	public Tamer() {
		...
		equipment.setChestplate(ItemBuilder.leatherTunic(Color.fromRGB(14557974)).addEnchant(Enchantment.THORNS, 1).cleanToItemSack());
		equipment.setLeggings(ItemBuilder.leatherPants(Color.fromRGB(3176419)).addEnchant(Enchantment.PROTECTION_ENVIRONMENTAL, 4).cleanToItemSack());
		equipment.setBoots(
			...
			public void run() {
				Manager.current().getCurrentGame().getAlivePlayers().forEach(gp -> {
					...
					pack.getEntities().forEach(entity -> {
						...
						if (target == null || (target instanceof Player playerTarget && !GamePlayer.getPlayer(playerTarget).isAlive())) {
							...
							Bukkit.getPluginManager().callEvent(
									new EntityTargetLivingEntityEvent(entity, nearest, EntityTargetEvent.TargetReason.CLOSEST_PLAYER)
							);
	...
	public void handleEntityTargetLivingEntityEvent(EntityTargetLivingEntityEvent ev) {
		...
		if (entity instanceof LivingEntity living && target instanceof Player player && validatePlayer(player, Heroes.TAMER)) {
			final TamerPack pack = getPack(player);
