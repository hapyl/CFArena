...
public enum Talents {
	...
	HAWKEYE_ARROW(new PassiveTalent("Hawkeye Arrow", "Fully charged shots while sneaking have 25% chance to fire hawkeye arrow which homes to nearby enemies.", Material.ENDER_EYE)),

	// Alchemist
	POTION(new RandomPotion()),
	CAULDRON(new CauldronAbility()),
	INTOXICATION(new PassiveTalent("Intoxication", "Drinking potions will increase &eIntoxication &7level that will decrease constantly. Keeping an eye on &eIntoxication &7level is a good idea, who knows what can happen...", Material.DRAGON_BREATH)),

	// Moonwalker
	MOONSLITE_PILLAR(new MoonslitePillar()),
	MOONSLITE_BOMB(new MoonSliteBomb()),
	TARGET(new PassiveTalent("Target", "Hold &e&lSNEAK &7to show your target block. Most of your abilities will spawn at the target block.", Material.SPECTRAL_ARROW)),

	// Hercules
	HERCULES_DASH(new HerculesShift()),
	HERCULES_UPDRAFT(new HerculesJump()),
	PLUNGE(new PassiveTalent("Plunge", "While airborne, &e&lSNEAK &7to perform plunging attack, dealing damage to nearby enemies.", Material.COARSE_DIRT)),

	// Mage
	MAGE_TRANSMISSION(new MageTransmission()),
	MAGE_TODO(null),
	SOUL_HARVEST(new PassiveTalent("Soul Harvest", "Deal &bmelee &7damage to gain soul fragment as fuel for your &e&lSoul &e&lEater&7's range attacks.", Material.SKELETON_SPAWN_EGG)),

	// Pytaria
	FLOWER_ESCAPE(new FlowerEscape()),
	FLOWER_BREEZE(new FlowerBreeze()),
	EXCELLENCY(new PassiveTalent("Excellency", "Pytaria gains &b30% &7damage boost per each &c10% &7of her missing &chealth &c❤&7.", Material.ROSE_BUSH)),
