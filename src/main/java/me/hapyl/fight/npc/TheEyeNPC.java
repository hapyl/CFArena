...
public class TheEyeNPC extends PersistentNPC implements QuestRegister {
    ...
    private Quest questFirstMeeting;
    
    public TheEyeNPC(@Nonnull Key key) {
        super(
                key, -5.5d, 62.0d, 6.5d, -135f, 0, Component.text("The Eye", Color.DEFAULT), Skin.of(
        ...
        setTagLayout(
                new TagLayout(
                        TagPart.of((_npc, _player) -> {
                            if (!questFirstMeeting.hasCompleted(_player)) {
                                return blink("ᴏ̨ᴜᴇsᴛ ᴀᴠᴀɪʟᴀʙʟᴇ");
                            }
                            
                            final EyeNotification notification = getFirstEyeNotification(_player);
                            
                            return notification != null ? blink(notification.aboveHead) : null;
                        }),
                        TagPart.name()
                )
        );
    ...
    public void registerQuests(@Nonnull QuestHandler handler) {
        handler.register(questFirstMeeting = new QuestFirstMeeting()); // Calls a runtime registry so has to be here
    }
    ...
    public enum EyeNotification {
        DAILY_REWARD_AVAILABLE(
                ...
                Component.text("Remember to claim your daily reward!"), Component.text("You can still claim a daily reward!")
        ),
        
        DAILY_BOND_COMPLETE(
                player -> CF.getProfile(player).getChallengeList().hasCompleteAndNonClaimed(),
                "ᴅᴀɪʟʏ ʙᴏɴᴅ ᴄᴏᴍᴘʟᴇᴛᴇ",
                Component.text("You have completed a bond but yet to claim it!"), Component.text("A bond has been experienced, your reward awaits.")
        ),
        
        CAN_CLAIM_RELIC_REWARDS(
                player -> CF.getDatabase(player).collectibleEntry.canClaimAnyTier(),
                "ᴄᴀɴ ᴄʟᴀɪᴍ ʀᴇʟɪᴄ ʀᴇᴡᴀʀᴅs",
                Component.text("You found some relics, come claim the rewards!"), Component.text("You have shown me the relics, get your rewards.")
        ),
        
        CAN_LEVEL_UP_STABILIZER(
                player -> CF.getDatabase(player).collectibleEntry.canLevelUpStabilizer(),
                "ᴄᴀɴ ʟᴇᴠᴇʟ ᴜᴘ ʀᴇʟɪᴄ sᴛᴀʙɪʟɪᴢᴇʀ",
                Component.text("There are enough relics for the stabilizer!"), Component.text("Relic stabilizer can be leveled up.")
        );
        
        private final Predicate<Player> predicate;
        private final String aboveHead;
        private final List<Component> chatStrings;
        
        EyeNotification(Predicate<Player> predicate, String aboveHead, Component... chatStrings) {
            this.predicate = predicate;
            this.aboveHead = aboveHead;
            this.chatStrings = List.copyOf(Arrays.asList(chatStrings));
        }
        
        @Nonnull
        public Component randomChatString() {
        ...
        public QuestFirstMeeting() {
            ...
            addObjective(new FindRelicQuestObjective(RELIC_ID) {
                ...
                public String getDescription() {
                    return "Find a Relic next to %s.".formatted(TheEyeNPC.this.getDefaultName());
                }
