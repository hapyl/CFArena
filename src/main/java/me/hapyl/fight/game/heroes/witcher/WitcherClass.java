...
public class WitcherClass extends Hero implements UIComponent, Disabled {
    
    private final PlayerMap<Combo> combos = PlayerMap.newMap();
    
    @DisplayField private final double defenseIncrease = 200;
    ...
    private class WitcherUltimate extends UltimateTalent {
        ...
        private final ModifierSource modifierSource = new ModifierSource(Key.ofString("witcher_ultimate"), true);
        ...
        public UltimateInstance newInstance(@Nonnull GamePlayer player, boolean isFullyCharged) {
            ...
            return builder()
                    .onExecute(() -> {
                        kven.startCooldown(player, 10000);
                        irden.startCooldown(player, 10000);
                        
                        player.getAttributes().addModifier(modifierSource, this, modifier -> modifier.of(AttributeType.DEFENSE, ModifierType.FLAT, defenseIncrease));
                    })
                    .onTick(tick -> {
                        irden.affect(player, player.getLocation(), tick);
                    })
                    .onEnd(() -> {
                        kven.startCooldown(player);
                        irden.startCooldown(player);
                    });
        }
    }
}
