...
import me.hapyl.fight.game.talents.techie.NeuralTheftPassive;
...
public class Techie extends Hero implements UIComplexComponent, Listener, PlayerDataHandler<TechieData>, DisplayFieldProvider {
            ...
            public void run() {
                for (GamePlayer player : getAlivePlayers()) {
                    ...
                    for (LivingGameEntity entity : data) {
                        for (GamePlayer teammate : player.getTeam().getPlayers()) {
    ...
    @Override
    public Saboteur getFirstTalent() {
        return TalentRegistry.SABOTEUR;
    }
    
    @Override
    public CipherLock getSecondTalent() {
        return TalentRegistry.CIPHER_LOCK;
    }
    
    @Override
    public NeuralTheftPassive getPassiveTalent() {
    ...
    public List<String> getStrings(@Nonnull GamePlayer player) {
        ...
        return List.of(Named.BUG.getPrefix() + "&f " + data.buggedSize());
    }
    ...
    private class TechieUltimate extends UltimateTalent {
        ...
        @DisplayField private final double attackSpeedDecrease = -50;
        
        private final ModifierSource modifierSource = new ModifierSource(Key.ofString("lockdown"));
        
        public TechieUltimate() {
            super(Techie.this, "Lockdown", 90);
            
            setDescription("""
                           Equip a &bhacking device&7; after a &nlong&7 &3casting time&7, overload all implanted %s&fs.
                           
                           Overloading the &fbugs&7 implodes them, causing affected enemies' &atalents&7 to be &dlocked&7.
                           &8&o;;Overloading bugs causes them to break.
                           
                           In addition, all &cenemies&7 lose &b&n{ultimateLosePercent}&7 of their %s.
                           """.formatted(Named.BUG, EnumResource.ENERGY)
            );
                    ...
                    public void onHack(@Nonnull GamePlayer player) {
                        ...
                        data.forEachAndRemove((entity) -> {
                            ...
                            entity.getAttributes().addModifier(
                                    modifierSource, lockdownTalentLockDuration, player, modifier -> modifier
                                            .of(AttributeType.SPEED, ModifierType.FLAT, attackSpeedDecrease)
                                            .of(AttributeType.ATTACK_SPEED, ModifierType.FLAT, attackSpeedDecrease)
                            );
