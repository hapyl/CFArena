...
public class Alchemist extends Hero implements UIComplexComponent, PlayerDataHandler<AlchemistData>, Listener {
    ...
    private final ModifierSource alchemicalMadnessSource = new ModifierSource(Key.ofString("alchemical_madness"));
    ...
    public Alchemist(@Nonnull Key key) {
        ...
        alchemicalMadnessEffects = new AlchemistEffect[][] {
                // Positive
                new AlchemistEffect[] {
                        ofMadness("made you &4&lStronger", Color.fromRGB(99, 5, 14), AttributeType.ATTACK, 0.5d, alchemicalMadnessPositiveDuration),
                        ofMadness("made you &b&lFaster", Color.fromRGB(99, 191, 247), AttributeType.SPEED, 0.05d, alchemicalMadnessPositiveDuration),
                        ofMadness("made you &3&lJump Higher", Color.fromRGB(47, 139, 196), AttributeType.JUMP_STRENGTH, 0.2d, alchemicalMadnessPositiveDuration),
                        ofMadness("&2&lProtected&e you", Color.fromRGB(22, 150, 8), AttributeType.DEFENSE, 0.5d, alchemicalMadnessPositiveDuration),
                        ofMadness(
                ...
                // Negative
                new AlchemistEffect[] {
                        ofMadness("made you &4&lWeaker", Color.fromRGB(99, 5, 14), AttributeType.ATTACK, -0.3d, alchemicalMadnessNegativeDuration),
                        ofMadness("made you &b&lSlower", Color.fromRGB(99, 191, 247), AttributeType.SPEED, -0.05d, alchemicalMadnessNegativeDuration),
                        ofMadness(
                                "made you &3&lJump Lower",
                                Color.fromRGB(47, 139, 196),
                                AttributeType.JUMP_STRENGTH,
                                -0.1d,
                                alchemicalMadnessNegativeDuration
                        ),
                        ofMadness(
                                "took your &2&lProtection&e away",
                                Color.fromRGB(22, 150, 8),
            ...
            public void run() {
                getAlivePlayers().forEach(player -> {
                    if (isToxinLevelBetween(player, 60, 80)) {
                        ...
                        player.decrementEnergy(1, player);
                    }
    ...
    @EventHandler
    public void handleExtraHealing(GameDamageEvent.Process ev) {
    ...
    @EventHandler
    public void handleAbyssCurse(GameDamageEvent.Process ev) {
        final LivingGameEntity entity = ev.getEntity();
    ...
    public List<String> getStrings(@Nonnull GamePlayer player) {
        ...
        return List.of(
                "%1$s&l%2$s %1$s%3$.0f%%".formatted(toxinColor, Named.ABYSS_CORROSION.getPrefix(), toxinLevel),
    ...
    private AlchemistEffect ofMadness(@Nonnull String name, @Nonnull Color color, @Nonnull AttributeType attributeType, double amount, int duration) {
        return ofMadness(
                name,
                color,
                (entity, alchemist) -> entity.getAttributes().addModifier(alchemicalMadnessSource, duration, alchemist, modifier -> modifier.of(attributeType, ModifierType.ADDITIVE, amount))
        );
    }
    
    private AlchemistEffect ofMadness(@Nonnull String name, @Nonnull Color color, @Nonnull BiConsumer<LivingGameEntity, GamePlayer> onHit) {
    ...
    private final class AlchemistEventHandler extends HeroEventHandler {
        ...
        public boolean handlePlayerClick(@Nonnull GamePlayer player, @Nonnull HotBarSlot slot) {
            ...
            if (data.state == AlchemistState.CHOOSING_POTION && slot == HotBarSlot.TALENT_1) {
                ...
                TalentRegistry.POTION.startCooldown(player, 5);
            ...
            if (potion != null) {
                setState(player, AlchemistState.NORMAL);
                talent.startCooldown(player, talent.getCooldown() + talent.castDuration);
