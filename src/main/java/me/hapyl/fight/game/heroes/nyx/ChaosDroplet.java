...
public class ChaosDroplet implements Removable, Located, ComparableTo<Item> {
    ...
    public ChaosDroplet(GamePlayer player, Location location) {
        this.nyx = player;
        this.entity = Entities.ARMOR_STAND.spawn(
                location, self -> {
                    self.setSmall(true);
                    ...
                    player.getTeam().getPlayers().forEach(teammate -> Glowing.setGlowing(teammate.getEntity(), self, GlowingColor.DARK_PURPLE));
                }
        );
    }
    ...
    public boolean affect(@Nonnull GamePlayer player, @Nonnull NyxData data) {
        ...
        if (this.nyx.isSelfOrTeammate(player)) {
            ...
            player.healRelativeToMaxHealth(healing, this.nyx);
            ...
            final double energyDecrease = TalentRegistry.CHAOS_GROUND.getEnergyDecrease();
            
            player.decrementEnergy(energyDecrease, nyx);
            nyx.incrementEnergy(energyDecrease);
