...
public class SwooperData extends PlayerData {
    ...
    @Nullable protected Location nestLocation;
    ...
    public void setStealthMode(boolean stealth) {
        ...
        // Enter mode
        if (stealth) {
            nestLocation = player.getLocation().add(0, 0.15, 0);
            ...
            // Start cooldown
            swooper.getPassiveTalent().startCooldown(player);
        ...
        player.sendTitle(Named.REFRACTION.getPrefixColored(), stealth ? "&aᴀᴄᴛɪᴠᴀᴛᴇᴅ" : "&cᴅᴇᴀᴄᴛɪᴠᴀᴛᴇᴅ", 0, 15, 5);
    }
    ...
    public void addHighlighted(@Nonnull LivingGameEntity entity) {
        ...
        Glowing.setGlowing(player.getEntity(), entity.getEntity(), GlowingColor.DARK_PURPLE);
    }
    ...
    public void removeHighlighted(@Nonnull LivingGameEntity entity) {
        ...
        Glowing.stopGlowing(player.getEntity(), entity.getEntity());
    }
    ...
    public String makeBars() {
        return Named.REFRACTION.getPrefixColored() + " " + makeBar(sneakTicks);
    }
    ...
    public void hidePlayer() {
        ...
        // Remove arrows
        player.getEntity().setArrowsInBody(0);
    }
    ...
    private void makeEquipmentMap(boolean real) {
        ...
        Bukkit.getOnlinePlayers().forEach(online -> {
            if (online.equals(player.getEntity()) || player.getTeam().isEntry(Entry.of(online))) {
                return;
            }
            
            online.sendEquipmentChange(player.getEntity(), map);
        });
    }
