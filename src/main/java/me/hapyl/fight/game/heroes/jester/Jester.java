...
public class Jester extends Hero implements Disabled, UIComponent, PlayerDataHandler<JesterData>, Listener {
    ...
    public String getString(@Nonnull GamePlayer player) {
        ...
        //
        // if (durationLeft > 0) {
        //     return "&d%s &f%s".formatted(Named.THE_JOKER.getPrefix(), CFUtils.formatTick(durationLeft));
        // }
    ...
    public class JesterUltimate extends UltimateTalent {
        ...
        public UltimateInstance newInstance(@Nonnull GamePlayer player, boolean isFullyCharged) {
            return builder()
                    .onCastStart(() -> {
                        Debug.info("onCastStat");
                    })
                    .onCastTick(i -> {
                        Debug.info("onCastTick = " + i);
                    })
                    .onCastEnd(() -> {
                        Debug.info("onCastEnd");
                    })
                    .onExecute(() -> {
                        Debug.info("onExecute");
                    })
                    .onTick(i -> {
                        Debug.info("onTick = " + i);
                    })
