...
public enum ArrowType implements Described {
    ...
    ELUSIVE("Elusive Arrows", "Bloom upon impact into &amultiple&7 arrows, dealing &cAoE &cdamage&7.") {
        @Override
        ...
        private void spawnArrow(GamePlayer player, Location location, Vector vector) {
            ...
            arrow.setShooter(player.getEntity());
            
            Manager.current().overrideProjectileDamageCause(arrow, DamageCause.ELUSIVE_ARROW);
        }
    },
    POISON_IVY(
            "Poison Ivy Arrow", """
        ...
        public void onShoot(GamePlayer player, Arrow arrow) {
            ...
            juju.getPlayerData(player).unequipeArrow();
    ...
    public String getTalentDescription(@Nonnull Timed timed) {
        return """
               ...
               &a🏹 &l%1$s
               &7 ;;%3$s
               """.formatted(getName(), Tick.round(timed.getDuration()), getDescription()
        );
    }
