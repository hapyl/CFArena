...
public final class Manager extends BukkitRunnable {
    ...
    // This might need optimizing if A LOT of entities since getting players is iterating over the whole map
    // Not we don't it's stored via UUID dumb ass
    ...
    public void createStartCountdown() {
        ...
        startCountdown = new StartCountdown() {
            ...
            public void onCountdownFinish() {
                startCountdown = null;
                createNewGameInstance(new GameInstance(currentMode, currentMap));
            }
        };
    }
    ...
    @Nonnull
    public PlayerProfile createProfile(@Nonnull @SensitiveParameter(
     ...
     */
    @Nonnull
    public <T extends GameEntity> T registerEntity(@Nonnull T entity, @Nullable Consumer<T> consumer) {
     ...
     */
    public void createNewGameInstance(@Nonnull GameInstance instance) {
        if (!canStartGame()) {
            return;
        }
        ...
        for (final GamePlayer gamePlayer : CF.getPlayers()) {
            ...
            // Teleport to the map
            player.teleport(instance.currentLevel().getLocation());

            // Glow teammates right after teleport
            gamePlayer.getTeam().glowTeammates();
        }

        final int revealIn = instance.currentLevel().getTimeBeforeReveal();
