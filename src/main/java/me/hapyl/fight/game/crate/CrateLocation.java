...
public class CrateLocation extends Location {
    ...
    public void onOpen(@Nonnull CrateLoot crateLoot) {
        ...
        final Cosmetic loot = crateLoot.getLoot();
    ...
    public void broadcastLoot(@Nonnull CrateLoot crateLoot) {
        final Player player = crateLoot.getPlayer();
        final Cosmetic cosmetic = crateLoot.getLoot();
        ...
        if (!isNew) {
            ...
            Chat.sendCenterMessage(player, rarity.getCompensation());
            Chat.sendMessage(player, "");
        }
    ...
    private void generateAndDisplayLoot(Player player, Collection<CrateLoot> collection, boolean duplicateStyle) {
        ...
        collection.forEach(crateLoot -> {
            ...
            if (isNewLoot) {
                ...
                compensation[0] += rarity.getCompensation();
            }
        ...
        if (compensation[0] > 0 && !duplicateStyle) {
            Chat.sendCenterMessage(player, Currency.COINS.formatProduct(compensation[0]));
        }
        ...
        }.runTaskLater(CF.getPlugin(), LOOT_DISPLAY_TIME);
    }
}
