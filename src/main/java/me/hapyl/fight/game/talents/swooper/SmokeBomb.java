...
public class SmokeBomb extends Talent {
    ...
    @DisplayField private final double speedIncrease = 10;
    
    private final double radiusScaled = (radius * radius) / 8.0d;
    private final ModifierSource modifierSource = new ModifierSource(Key.ofString("smoke_bomb"), true);
    ...
    public SmokeBomb(@Nonnull Key key) {
        ...
        setMaterial(Material.ENDERMITE_SPAWN_EGG);
        setCooldownSec(20);
        setDurationSec(5);
    }
    ...
    public @Nullable Response execute(@Nonnull GamePlayer player) {
        ...
        player.getAttributes().addModifier(modifierSource, this, modifier -> modifier.of(AttributeType.SPEED, ModifierType.FLAT, speedIncrease));
        
        new GameTask() {
            private final RetainSet<LivingGameEntity> retainSet = new RetainSet<>() {
                @Override
                public void onAdd(@Nonnull LivingGameEntity entity) {
                    entity.setGlowingFor(player, GlowingColor.DARK_GRAY);
                }
