...
public class RoninDash extends Talent {
    ...
    private final ModifierSource modifierSource = new ModifierSource(Key.ofString("ronin_dash"), true);
            ...
            public boolean tick(int tick, int step) {
                ...
                Collect.nearbyEntities(player.getLocation(), 1.5d, player::isNotSelfOrTeammate)
                       .forEach(entity -> {
                           ...
                           entity.getAttributes().addModifier(modifierSource, speedDecreaseDuration, player, modifier -> modifier.of(AttributeType.SPEED, ModifierType.FLAT, speedDecrease));
