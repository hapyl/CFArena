...
public class AlchemicalCauldron extends TickingGameTask implements Removable {
    ...
    public AlchemicalCauldron(@Nonnull GamePlayer player, @Nonnull Location location, @Nonnull CauldronAbility ability) {
        ...
        this.hitBoxEntity = Hitbox.create(
                location, ability.health, 2, new Hitbox() {
                    @Nonnull
                    @Override
                    public String getName() {
                        return "Alchemical Cauldron";
                    }
        ...
        player.getTeam().addEntry(hitBoxEntity.getEntry());
        
        this.hologram = Hologram.ofArmorStand(location.clone().add(0, 0.75d, 0));
        
        for (GamePlayer teamMates : player.getTeam().getPlayers()) {
            this.hologram.show(teamMates.getEntity());
        }
