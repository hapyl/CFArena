...
public class BlindingCurse extends DarkMageTalent {
    ...
    private final ModifierSource modifierSource = new ModifierSource(Key.ofString("darkness"));
        ...
        new GameTask() {
            ...
            private void checkHit(Location location) {
                Collect.nearbyEntities(location, 1.0d).forEach(entity -> {
                    ...
                    entity.getAttributes().addModifier(modifierSource, impairDuration, player, modifier -> modifier.of(AttributeType.SPEED, ModifierType.FLAT, speedReduction));
    ...
    public void scaryWither(@Nonnull GamePlayer player) {
        ...
        final Wither wither = Entities.WITHER.spawn(
                witherLocation, self -> {
                    self.setSilent(true);
                    ...
                    self.setTarget(Wither.Head.LEFT, player.getEntity());
                    self.setTarget(Wither.Head.RIGHT, player.getEntity());
                }
        );
