...
public class EngineerTurret extends EngineerTalent {
    ...
    private final DisplayData sentry1 = BDEngine.parse(
    ...
    public EngineerTurret() {
        super(Key.ofString("engineer_sentry"), "Sentry", 6);
    ...
    public Construct create(@Nonnull GamePlayer player, @Nonnull Location location) {
        DisplayEntity displayEntity = sentry1.spawn(location);
        return new Construct(player, location, this) {
            ...
            public void onTick() {
                ...
                if (nearestEntity == null) {
                    ...
                    final BlockDisplay sentrySpin = displayEntity.getHead();
                    final Location location = stand.getLocation();

                    location.setYaw(location.getYaw() + 5);

                    sentrySpin.teleport(location);
                    stand.teleport(location);

                    return;

                    //TODO - make the sentry spin, because i suck
                }
