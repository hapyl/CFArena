...
public class EnderPassive extends PassiveTalent {
    ...
    public void handleTeleport(@Nonnull GamePlayer gamePlayer) {
        gamePlayer.heal(gamePlayer.getMaxHealth() * healing);

        final EntityAttributes attributes = gamePlayer.getAttributes();
        attributes.increaseTemporary(Temper.ENDER_TELEPORT, AttributeType.ATTACK, attributes.get(AttributeType.ATTACK) * attackBoost, attackBoostDuration);
