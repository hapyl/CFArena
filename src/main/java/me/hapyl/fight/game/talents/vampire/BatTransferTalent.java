...
public class BatTransferTalent extends Talent {
    ...
    public BatTransferTalent(@Nonnull Key key) {
        ...
        setDescription("""
                       ...
                       &8&o;;Press jump or sneak button to manually transform back.
                       """);
        ...
        public void run(int tick) {
            ...
            Collect.nearbyEntities(location, hitBoxSize, player::isNotSelfOrTeammate)
                   .forEach(entity -> {
                       ...
                       if (attributes.hasModifier(modifierSource)) {
                           return;
                       }
                       
                       entity.damage(damage, player, DamageCause.BAT_BITE);
                       attributes.addModifier(modifierSource, impairDuration, player, modifier -> modifier.of(AttributeType.MAX_HEALTH, ModifierType.ADDITIVE, healthDecrease));
