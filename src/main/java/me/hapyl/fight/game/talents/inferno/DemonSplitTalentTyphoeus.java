...
public class DemonSplitTalentTyphoeus extends DemonSplitTalent {
    ...
    @DisplayField(percentage = true) private final double trailOfFireDamage = 0.05d;
    ...
    private final EntityCooldown cooldown = new EntityCooldown(Key.ofString("trail_of_fire"), trailOfFireDamagePeriod * 50L);
    ...
    public ReformDescription describeReform() {
        return new ReformDescription(
                "Repeat", """
                          &nrepeat&7 the &4damage&7 you dealt in the last &b%s&7 multiplied by &cx%.1f&7 as %s.
    ...
    public DemonInstance newInstance(@Nonnull GamePlayer player) {
        return new DemonInstance() {
            private final Queue<InfernoFire> fires = new LinkedList<>();
            ...
            public void onTick(@Nonnull GamePlayer player, @Nonnull InfernoData data, int tick) {
                ...
                if (block.getType() == Material.AIR && down.isSolid()) {
                    final InfernoFire fire = TalentRegistry.FIRE_PIT.createFire(
                            block.getLocation(), InfernoFire.Type.FIRE, entity -> {
                                if (player.isSelfOrTeammate(entity) || entity.hasCooldown(cooldown)) {
                                    return;
                                }
                                
                                final double damage = entity.getMaxHealth() * trailOfFireDamage;
                ...
                if (tick > 0 && tick % trailOfFireExtinguishPeriod == 0) {
                    final InfernoFire last = fires.poll();
            ...
            @Override
            public void remove() {
                fires.forEach(InfernoFire::remove);
                fires.clear();
            }
        };
    }
