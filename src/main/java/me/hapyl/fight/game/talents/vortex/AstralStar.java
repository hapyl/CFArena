...
public class AstralStar implements Ticking {
    ...
    private GlowingColor currentColor;
    private StarState state;
    
    public AstralStar(GamePlayer player, Location location, VortexStarTalent talent, double healthSacrifice) {
        this.player = player;
        this.talent = talent;
        ...
        this.entity = CF.createEntity(
                location.subtract(0, 0.25, 0), Entities.SLIME, slime -> {
                    ...
                    attributes.setMaxHealth(healthSacrifice);
                    ...
                    final LivingGameEntity entity = new LivingGameEntity(slime, attributes) {
                        ...
                        public void onDamageTaken(@Nonnull DamageInstance instance) {
                            ...
                            instance.overrideDamage(Math.min(instance.getDamage(), healthSacrifice * 0.2));
                        }
    ...
    public void tick() {
        ...
        // SFx
        if (ticks % 20 == 0) {
            player.getEntity().playSound(location, Sound.BLOCK_BEACON_POWER_SELECT, SoundCategory.RECORDS, 0.5f, 0.75f);
        }
    ...
    public void setColorGlobal(@Nonnull GlowingColor color) {
        CF.getPlayers().forEach(player -> this.displayEntity.forEach(entity -> {
            Glowing.setGlowing(player.getEntity(), entity, color);
        }));
    }
