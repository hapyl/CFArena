...
public class Icicles extends Talent {
    ...
    @DisplayField private final double critChanceReduction = -25;
    ...
    private final ModifierSource source = new ModifierSource(Key.ofString("frostfall"));
    ...
    public void affect(@Nonnull GamePlayer player, @Nonnull Location location) {
        Collect.nearbyEntities(location, distance).forEach(entity -> {
            ...
            entity.getAttributes().addModifier(
                    source, debuffDuration, player, modifier -> modifier
                            .of(AttributeType.CRIT_DAMAGE, ModifierType.FLAT, critChanceReduction)
                            .of(AttributeType.CRIT_CHANCE, ModifierType.FLAT, critDamageReduction)
                            .of(AttributeType.ATTACK, ModifierType.MULTIPLICATIVE, attackReduction)
            );
            
            entity.addPotionEffect(PotionEffectType.SLOWNESS, 1, 20);
        });
    ...
    public @Nullable Response execute(@Nonnull GamePlayer player) {
        final LivingGameEntity targetEntity = Collect.targetEntityDot(player, 10, 0.7d, entity -> !player.isSelfOrTeammate(entity));
        ...
        new TickingGameTask() {
            ...
            private void spawnFallingBlock() {
                ...
                fallingBlocks.add(Entities.ARMOR_STAND_MARKER.spawn(
                        location, self -> {
