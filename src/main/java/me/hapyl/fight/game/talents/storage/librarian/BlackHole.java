...
public class BlackHole extends LibrarianTalent {
    ...
    public BlackHole() {
        ...
        addDescription(
                "Creates a black hole at your target block. Pulling enemies in and dealing <scaled> damage per second based on &cGrimoire &7level."
        );
    ...
    public Response executeGrimoire(Player player) {
        ...
        GameTask.runTaskTimerTimes((task, tick) -> {
            ...
            Utils.getEntitiesInRange(location, suckRadius).forEach(entity -> {
                ...
                if (tick % 20 == 0) {
                    GamePlayer.damageEntity(entity, getCurrentValue(Heroes.LIBRARIAN.getHero(Librarian.class).getGrimoireLevel(player)));
