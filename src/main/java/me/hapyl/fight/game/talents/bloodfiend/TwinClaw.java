package me.hapyl.fight.game.talents.bloodfiend;

import me.hapyl.eterna.module.block.display.BDEngine;
import me.hapyl.eterna.module.block.display.DisplayData;
import me.hapyl.eterna.module.block.display.DisplayEntity;
import me.hapyl.eterna.module.entity.Entities;
import me.hapyl.fight.game.damage.DamageCause;
import me.hapyl.fight.game.entity.GamePlayer;
import me.hapyl.fight.game.entity.LivingGameEntity;
import me.hapyl.fight.game.heroes.HeroRegistry;
import me.hapyl.fight.game.heroes.bloodfield.Bloodfiend;
import me.hapyl.fight.game.heroes.bloodfield.BloodfiendData;
import me.hapyl.fight.game.profile.PlayerProfile;
import me.hapyl.fight.game.profile.data.AchievementData;
import me.hapyl.fight.game.profile.data.Type;
import me.hapyl.fight.game.talents.TalentRegistry;
import me.hapyl.fight.game.talents.bloodfiend.chalice.BloodChaliceTalent;
import me.hapyl.fight.game.talents.bloodfiend.taunt.Taunt;
import me.hapyl.fight.game.task.TickingGameTask;
import me.hapyl.fight.registry.Registries;
import me.hapyl.fight.util.Collect;
import org.bukkit.Location;
import org.bukkit.Particle;
import org.bukkit.Sound;
import org.bukkit.entity.ArmorStand;
import org.bukkit.util.Vector;

public class TwinClaw extends TickingGameTask {
    
    private final static DisplayData DISPLAY_DATA = BDEngine.parse(
            "{Passengers:[{id:\"minecraft:block_display\",block_state:{Name:\"minecraft:orange_terracotta\",Properties:{}},transformation:[1.0000f,0.0000f,0.0000f,-0.5000f,0.0000f,1.0000f,0.0000f,0.0000f,0.0000f,0.0000f,1.0000f,-0.5000f,0.0000f,0.0000f,0.0000f,1.0000f]},{id:\"minecraft:block_display\",block_state:{Name:\"minecraft:orange_terracotta\",Properties:{}},transformation:[1.0000f,0.0000f,0.0000f,-0.5000f,0.0000f,1.0000f,0.0000f,0.0000f,0.0000f,0.0000f,1.0000f,-0.5000f,0.0000f,0.0000f,0.0000f,1.0000f]},{id:\"minecraft:block_display\",block_state:{Name:\"minecraft:orange_terracotta\",Properties:{}},transformation:[0.6250f,0.0000f,0.0000f,-0.3125f,0.0000f,0.6875f,0.0000f,2.0000f,0.0000f,0.0000f,0.5000f,0.1250f,0.0000f,0.0000f,0.0000f,1.0000f]},{id:\"minecraft:block_display\",block_state:{Name:\"minecraft:red_terracotta\",Properties:{}},transformation:[0.7500f,0.0000f,0.0000f,-0.3750f,0.0000f,1.0000f,0.0000f,1.0000f,0.0000f,0.0000f,0.8125f,-0.2500f,0.0000f,0.0000f,0.0000f,1.0000f]},{id:\"minecraft:block_display\",block_state:{Name:\"minecraft:terracotta\",Properties:{}},transformation:[0.5000f,0.0000f,0.0000f,-0.2500f,0.0000f,0.2210f,-0.4419f,2.8750f,0.0000f,0.2210f,0.4419f,-0.1250f,0.0000f,0.0000f,0.0000f,1.0000f]}]}"
    );
    
    private final GamePlayer player;
    private final Vector vector;
    private final ArmorStand entity;
    private final int duration;
    private final DisplayEntity displayEntity;
    
    TwinClaw(GamePlayer player, Location startLocation, Vector vector, int duration) {
        this.player = player;
        this.vector = vector;
        this.duration = duration;
        
        final Vector direction = startLocation.getDirection();
        
        //direction.setY(0.0d);
        startLocation.setDirection(direction);
        
        entity = Entities.ARMOR_STAND_MARKER.spawn(
                startLocation, self -> {
                    self.setGravity(false);
                    self.setSmall(true);
                    self.setInvulnerable(true);
                    self.setInvisible(true);
                }
        );
        
        displayEntity = DISPLAY_DATA.spawnInterpolated(startLocation);
        
        // Fx
        player.playWorldSound(Sound.ENTITY_ZOMBIE_BREAK_WOODEN_DOOR, 0.75f);
        
        runTaskTimer(0, 1);
    }
    
    @Override
    public void run(int tick) {
        if (tick > duration) {
            remove();
            return;
        }
        
        final Location location = entity.getLocation();
        location.add(vector);
        
        if (location.getBlock().getType().isOccluding()) {
            remove();
            return;
        }
        
        entity.teleport(location);
        displayEntity.teleport(location);
        
        // Damage entity
        final LivingGameEntity nearestEntity = Collect.nearestEntity(location, 1, entity -> !player.isSelfOrTeammate(entity));
        
        // If no entity was hit, test for chalice
        if (nearestEntity == null) {
            final BloodChaliceTalent chaliceTalent = TalentRegistry.BLOOD_CHALICE;
            final Taunt chalice = chaliceTalent.getTaunt(player);
            
            if (chalice == null) {
                return;
            }
            
            final double distanceToChalice = location.distanceSquared(chalice.location());
            
            if (distanceToChalice <= 1.5) {
                chalice.remove();
                remove();
                
                // Fx
                player.spawnWorldParticle(location, Particle.POOF, 5, 0.1, 0.2, 0.1, 0.05f);
                player.playWorldSound(location, Sound.BLOCK_GLASS_BREAK, 0.0f);
            }
            
            return;
        }
        
        final TwinClaws talent = TalentRegistry.TWIN_CLAWS;
        double damage = talent.twinClawDamage;
        
        final Bloodfiend bloodfiend = HeroRegistry.BLOODFIEND;
        final BloodfiendData data = bloodfiend.getPlayerData(player);
        
        if (data.isSuckedEntity(nearestEntity)) {
            damage *= talent.bittenDamageIncrease;
        }
        
        nearestEntity.damage(damage, player, DamageCause.TWINCLAW);
        remove();
        
        // Achievement
        final PlayerProfile profile = player.getProfile();
        final AchievementData achievementData = profile.getPlayerData()
                                                       .getAchievementData(Registries.achievements().BLOODFIEND_THEY_ARE_TWINS_ALRIGHT);
        
        final int useTime = achievementData.checkExpire(5000).increment(Type.USE_TIME, 1);
        
        if (useTime >= 2) {
            achievementData.completeAchievement();
        }
    }
    
    public void remove() {
        final Location location = entity.getLocation().add(0.0d, 1.5d, 0.0d);
        
        entity.remove();
        displayEntity.remove();
        cancel();
        
        // Fx
        player.spawnWorldParticle(location, Particle.POOF, 20, 0.5d, 0.6d, 0.5d, 0.1f);
    }
}
