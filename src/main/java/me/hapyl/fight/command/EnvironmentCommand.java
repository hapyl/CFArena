...
public class EnvironmentCommand extends CFCommand {
    ...
    protected void execute(@Nonnull Player player, @Nonnull ArgumentList args, @Nonnull PlayerRank rank) {
        ...
        if (args.length == 0) {
            Notifier.success(player, "Current environment values:");
            ...
            if (property == null) {
                Notifier.error(player, "Invalid property: {%s}!".formatted(name));
                return;
            }
            ...
            if (args.length == 1) {
                Notifier.success(player, "Environment value of {%s} is {%s}.".formatted(propertyName, propertyValue));
            }
            else {
