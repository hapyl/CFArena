...
public class ParticleCommand extends SimplePlayerAdminCommand {
	...
	protected void execute(Player player, String[] args) {
		...
		final Particle particle = Enums.byName(Particle.class, args[0]);
		...
		final int amount = Numbers.getInt(args.length >= 2 ? args[1] : 1);
		final double offsetX = Numbers.getDouble(args.length >= 3 ? args[2] : 0.0d);
		final double offsetY = Numbers.getDouble(args.length >= 4 ? args[3] : 0.0d);
		final double offsetZ = Numbers.getDouble(args.length >= 5 ? args[4] : 0.0d);
		final float speed = Numbers.getFloat(args.length >= 6 ? args[5] : 0.0d);
